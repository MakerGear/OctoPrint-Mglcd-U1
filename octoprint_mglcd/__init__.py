...
class NextionPlugin(octoprint.plugin.StartupPlugin,
	...
	def __init__(self):
		...
		self.disableMoveProtections = False
	...
	def flashFirmware(self):
		...
		self.firmwareLocation = self._basefolder+"/static/supportfiles/nextion_uploader/u1-v3-0130.tft"
	...
	def populateDeleteList(self):
		...
		for file in sorted(list(self.deleteFiles['local'].keys()), key = lambda x: x.lower()):
			if self.deleteFiles['local'][file]['type'] == 'folder':
				self.deleteList[i] = {'name' : self.deleteFiles['local'][file]['name'] ,
	...
	def processMessage(self, lineRaw):
		...
		if "button" in line:
			...
			if splitLine[1] in ("x0", "x1", "y", "z", "t0", "t1"):
				...
				if self.disableMoveProtections:
					self._printer.commands(["M400", "M564 S0 H0"])
				self._printer.jog(moveDict, speed = speed)
				if self.disableMoveProtections:
					self._printer.commands(["M400", "M564 S1 H1"])
			...
			if "button fileMenu" in line:
				...
				try:
					...
					if fileButton.isdigit():
						...
						try:
							if self.fileList[int(fileButton)+self.fileListLocation]['type'] == 'folder':
								if self.fileList[int(fileButton)+self.fileListLocation]['name'] == 'up':
									self.navigateFolderUp()
									self.populatePrintList()

								else:
									self.currentFolder = self.fileList[int(fileButton)+self.fileListLocation]['path']
									self.currentPath = self.fileList[int(fileButton)+self.fileListLocation]['path']
									self.fileListLocation = 0
									self.populatePrintList()
							else:
			...
			if line == "button homing disabled":
				...
				self.disableMoveProtections = True
			
			if line == "button collect logs": 
				self.collectLogs()

			if line == "button probe deploy":
				self._printer.commands("M401")

			if line == "button probe retract":
				self._printer.commands("M402")

			if line == "button probe test":
				self._printer.commands("M280 P0 S160")
			...
			if "button deleteMenu" in line:
				...
				try:
					...
					if fileButton.isdigit():
						self._logger.info(self.previousFolderList)
						try:
							if self.deleteList[int(fileButton)+self.deleteListLocation]['type'] == 'folder':
								if self.deleteList[int(fileButton)+self.deleteListLocation]['name'] == 'up':
									self.navigateFolderUp()
									self.populateDeleteList()

								else:
									self.currentFolder = self.deleteList[int(fileButton)+self.deleteListLocation]['path']
									self.currentPath = self.deleteList[int(fileButton)+self.deleteListLocation]['path']
									self.deleteListLocation = 0
									self.populateDeleteList()
							...
							self._logger.info("Keyerror when selecting file to delete: "+str(e))
							traceback.print_exc()
							self._logger.info(line)

						except Exception as e:
							self._logger.info("General exception when selecting file to delete: "+str(e))
							traceback.print_exc()
							self._logger.info(line)
			...
			if "button delete confirm" in line:
				# self._file_manager.remove_file('local', self.deleteList[int(fileButton)+self.deleteListLocation][1]['path'])
				# self.populateDeleteList()
				self._file_manager.remove_file('local', self.fileToDelete['path'])
				self.fileToDelete = None
				self.populateDeleteList()
